...
in vec4 vertexColor;
in vec2 texCoord0;
// Note: 'normal' variable is kept if your VSH provides it, 
// though not used for lighting here to maintain consistency.
...
void main() {
    ...
    // 3. Brillo global inicial
    color.rgb *= 0.65;

    // 4. Aplicar Vibranza
    color.rgb = applyVibrance(color.rgb, 0 * SATURATION_STRENGTH);

    // 5. Lógica de sombreado por brillo de vértice
